...
from magma.train_loop import (
    eval_step,
    final_inference_step,
    train_step,
)


def _load_img_cpt_datasets(dataset_dir, tokenizer, transforms, load_data_in_memory=True, config=None):
...
def get_pretraining_datasets(config, tokenizer, transforms):
    ...
    train_dataset = _load_img_cpt_datasets(
        config.train_dataset_dir, tokenizer, transforms, load_data_in_memory=True, config=config
    )
    ...
    if config.eval_dataset_dir is None:
        ...
        eval_dataset = _load_img_cpt_datasets(
            config.eval_dataset_dir, tokenizer, transforms, load_data_in_memory=True, config=config
        )
...
if __name__ == "__main__":
    ...
    model.config.final_infernece = True
    ...
    # load data:
    config.train_dataset_dir = '/home/duan/magma/dataset/scicap/test'
    ...
    pbar = tqdm(
        range(0, int(len(train_dataset)/config.batch_size/torch.cuda.device_count())),
        desc="inferencing...",
        initial=0,
        total=int(len(train_dataset)/config.batch_size/torch.cuda.device_count()),
        disable=not is_main(),
    )
    """wandb_init(
    ...
    for i in pbar:
        ...
        if global_step % config.eval_every == 0:
            ...
            with torch.no_grad():
                ...
                ##### inference:"""
        with torch.no_grad():
            image_grid, captions, gold = final_inference_step(config, train_loader, model_engine)
            pred.append([captions, gold])
        """wandb_log(
    ...
    torch.save(pred, config.save+'/test_output.pt'+str(args.local_rank))
